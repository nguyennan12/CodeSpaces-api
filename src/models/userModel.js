...
const INVALID_UPDATE_FIELDS = ['email', 'password', 'createdAt', 'username']
...
const findOneByEmail = async (emailUser) => {
  try {
    return await GET_DB().collection(USER_COLLECTION_NAME).findOne({ email: emailUser })
  } catch (error) { throw new Error(error) }
}
